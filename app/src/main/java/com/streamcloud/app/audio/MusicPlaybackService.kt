...
class MusicPlaybackService : MediaLibraryService() {
    ...
    private fun resolveMusicDataSpec(dataSpec: DataSpec): DataSpec {
        ...
        // Only skip resolution for fully-completed ExoPlayer downloads. The player
        // must use the download request's exact cache key: new requests are keyed
        // by video ID while the playback item uses the full watch URL. Leaving the
        // watch URL key here misses the completed cache and feeds YouTube's HTML
        // page to Media3, which appears as an unsupported-container error.
        com.streamcloud.app.data.downloads.YtMusicDownloadUtil
            .completedDownloadCacheKey(watchUrl)
            ?.let { completedCacheKey ->
                return dataSpec.buildUpon()
                    .setKey(completedCacheKey)
                    .build()
            }
