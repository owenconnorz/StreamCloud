...
object NuvioRuntime {
    ...
    private fun buildPolyfillCode(scriptKey: String): String = """
        ...
        // ── Common Node.js/browser globals providers expect but QuickJS omits ───
        // process — providers check process.env, process.platform, etc.
        ...
        // setTimeout/clearTimeout — providers use them for delays and retry loops.
        // QuickJS has no native timer in our embedding; stub them so providers
        // that do `await new Promise(r => setTimeout(r, 100))` do not hang.
        ...
        if (typeof AbortController === 'undefined') {
            ...
            globalThis.AbortSignal.timeout = function() { return new AbortSignal(); };
            globalThis.AbortController = function() {
                this.signal = new AbortSignal();
                this.abort = function(reason) {
                    this.signal.aborted = true;
                    this.signal._listeners.forEach(function(fn) { try { fn(); } catch(e) {} });
                };
            };
        }
        // URL / URLSearchParams
        if (typeof URLSearchParams === 'undefined') {
            globalThis.URLSearchParams = function(init) {
                ...
                if (typeof init === 'string') {
                    (init.charAt(0) === '?' ? init.slice(1) : init).split('&').forEach(function(kv) {
                        if (!kv) return;
                        var eq = kv.indexOf('=');
                    ...
                    Object.keys(init).forEach(function(k) { p[k] = String(init[k]); });
                }
                this._p = p;
                this.get    = function(k) { return p.hasOwnProperty(k) ? p[k] : null; };
                this.set    = function(k, v) { p[k] = String(v); };
                this.append = function(k, v) { p[k] = String(v); };
                this.has    = function(k) { return p.hasOwnProperty(k); };
                this.delete = function(k) { delete p[k]; };
                this.toString = function() {
                    return Object.keys(p).map(function(k) {
                        return encodeURIComponent(k) + '=' + encodeURIComponent(p[k]);
                    }).join('&');
                };
                this.entries = function() { return Object.entries(p)[Symbol.iterator](); };
                this.keys    = function() { return Object.keys(p)[Symbol.iterator](); };
                this.values  = function() { return Object.values(p)[Symbol.iterator](); };
                this.forEach = function(cb) { Object.keys(p).forEach(function(k) { cb(p[k], k); }); };
            };
        }
        if (typeof URL === 'undefined') {
            globalThis.URL = function(href, base) {
                ...
                if (base && !/^https?:\/\//i.test(href)) {
                    full = base.replace(/\/+${'$'}/, '') + (href.charAt(0) === '/' ? '' : '/') + href;
                }
                var parsed = (typeof __parse_url !== 'undefined') ? __parse_url(full) : {};
                this.href     = full;
        ...
        // navigator — agents and language detection
        ...
        if (typeof TextEncoder === 'undefined') {
            ...
            globalThis.TextEncoder.prototype.encode = function(s) {
                var buf = []; for (var i = 0; i < s.length; i++) buf.push(s.charCodeAt(i) & 0xff); return new Uint8Array(buf);
            };
            globalThis.TextDecoder = function() {};
            globalThis.TextDecoder.prototype.decode = function(buf) {
                var s = ''; var arr = buf instanceof Uint8Array ? buf : new Uint8Array(buf);
                for (var i = 0; i < arr.length; i++) s += String.fromCharCode(arr[i]); return s;
            };
        }
        // Uint8Array / ArrayBuffer — QuickJS has them but guard for completeness
        if (typeof Uint8Array === 'undefined') { globalThis.Uint8Array = Array; globalThis.ArrayBuffer = Array; }
