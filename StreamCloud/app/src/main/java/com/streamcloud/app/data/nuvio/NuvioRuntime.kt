        ...
        return try {
            ...
            quickJs(Dispatchers.IO) {
                ...
                val directResult = evaluate<Any?>(buildString {
                    ...
                    appendLine("  // ── Globals — matching NuvioMobile-Enhanced pluginService.ts exactly ───")
                    ...
                    appendLine("  };")
                    appendLine("  var PRIMARY_KEY = undefined;")
                    appendLine("  var TMDB_API_KEY = ${jsString(com.streamcloud.app.BuildConfig.TMDB_API_KEY)};")
                    appendLine("  var SCRAPER_SETTINGS = {};")
                    appendLine("  var SCRAPER_ID = ${jsString(scriptKey)};")
                    ...
                    appendLine("  // ── Provider code — no wrapper, exactly as NuvioMobile runs it ─────────")
                    ...
                    appendLine("    return '[]';")
                    appendLine("  }")
                    appendLine("  // ── Call with 4 positional args (NuvioMobile exact convention) ──────────")
                    appendLine("  var __result = __fn(params.tmdbId, params.mediaType, params.season, params.episode);")
                    ...
                    appendLine("      console.error('[provider] error:', (__e && __e.message) || String(__e));")
                    appendLine("      __capture_result('[]');")
                    appendLine("    });")
